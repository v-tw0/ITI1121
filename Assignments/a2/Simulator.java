...
public class Simulator {
	...
	public void simulate() {
		...
		/* ArrivalPerSec variable to determine if a car arrives or not */
		Rational ArrivalPerSec = new Rational(0, 3600);

		/* Queue for temporary use */
		Queue<Spot> incomingQueueSize = new LinkedQueue<>();

		/* Variable declaration */
		int duration;
		int lotHeight = lot.getNumRows();
		int lotWidth = lot.getNumSpotsPerRow();
		int carOccupancy = 0;
		this.clock = 0;
		this.steps = 24 * 3600;
		...
		while (clock < steps) {
			/* Add one increment to arrival per second variable */
			ArrivalPerSec = ArrivalPerSec.plus(this.probabilityOfArrivalPerSec);

			/* If ArrivalPerSec is a multiple of itself (i.e. denominator is 1),
			then generate a new random car at that time and add to incoming queue */
			
			
			if((ArrivalPerSec.denominator()) == 1) {
				Car car = RandomGenerator.generateRandomCar();
				Spot spot = new Spot(car, clock);
				incomingQueue.enqueue(spot);
			}
			
			/* For every car c parked in the lot, calculate the duration that c has been parked */
			for(int i = 0; i < lotHeight; i++) {
				for(int j = 0; j < lotWidth; j++) {

					/* If spot isn't empty */
					if(lot.getSpotAt(i,j) != null) {
						Spot s = lot.getSpotAt(i,j);
						//System.out.println(s.getTimestamp());
						duration = clock - s.getTimestamp();

						/* If duration == 8h, remove from lot and add to outgoingQueue */
						if(duration == MAX_PARKING_DURATION && s.getCar() != null) {
							carOccupancy--;
							outgoingQueue.enqueue(s);
							System.out.println(s.getCar() + " EXITED at timestep " + clock + "; occupancy is at " + carOccupancy);
							lot.remove(i,j);
						}

						/* If duration < 8h, check if car leaves at exact moment */
						if(duration < MAX_PARKING_DURATION && s.getCar() != null) {
			...
			if(!incomingQueue.isEmpty()) {
				Spot s1 = incomingQueue.dequeue();
				Car c1 = s1.getCar();

				/* If parking is successful */
				if(lot.attemptParking(c1, s1.getTimestamp())) {
					carOccupancy++;
					System.out.println(c1 + " ENTERED at timestep " + clock + "; occupancy is at " + carOccupancy);
				}
				/* Else if parking wasn't successful, place back in queue */
				else {
					incomingQueue.enqueue(s1);
				}
			}
