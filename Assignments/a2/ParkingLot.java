...
public class ParkingLot {
	...
	public void park(int i, int j, Car c, int timestamp) {
		...
		occupancy[i][j].setTimestamp(timestamp);
	}
	...
	public Spot remove(int i, int j) {
		...
		else if(i > occupancy.length || j > occupancy[0].length || i < 0 || j < 0) {
			return null;
		}
		else {
			occupancy[i][j].setCar(null);
	...
	public boolean attemptParking(Car c, int timestamp) {
		...
		/* For loops searching for a valid parking spot in the lot. */
		for(int i = 0; i < lotDesign.length; i++) {
			for (int j = 0; j < lotDesign[0].length; j++) {
				/* If cartype not matching, continue loop */
				if(carType != (lotDesign[i][j])) {
					continue;
				}
				/* If cartype matches and spot is empty, set occupancy car and timestamp */
				else if(carType.equals(lotDesign[i][j]) == true && occupancy[i][j] == null) {
					occupancy[i][j] = new Spot(c, timestamp);
					return true;
				}
			}
		}
		/* Return false when no spot is found */
		return false;
	}
