...
public class SinglyLinkedList<E> implements List<E>, Iterable<E> {
    private int count(Node <E> p, E fromElement, E toElement, int count) {
        /* null to anything is 0, so if head is null, return 0 */
        if(p == null) {
            return 0;
        }
        /* if the count is 1 and the head equals toElement, return 1 */
        if(p.value.equals(toElement) && count == 1) {
            return 1;
        }

        /* if the count is 0 and the head equals fromElement */
        if(p.value.equals(fromElement) && count == 0) {

            /* set count to 1 */
            count = 1;

            /* if both elements are equal to each other, return 1 */
            if(fromElement.equals(toElement)) {
                return 1;
            }

            /* else call method and increment by 1 */
            else {
                return 1+count(p.next,fromElement,toElement,count);
            }
        }

        /* declare integer using next value */
        int val = count(p.next,fromElement,toElement,count);

        /* if count equals 1, return the value + 1 */
        if(count == 1) {
            return 1+val;
        }
    ...
    public int count(E fromElement, E toElement) {
        /* Call count method with head of list & initial count of 0 */
